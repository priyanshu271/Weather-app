...
export class WeatherDataService {
  ...
  public async getWeather(cityName: string): Promise<WeatherInfo> {
    try {
      const geoUrl = `http://api.openweathermap.org/geo/1.0/direct?q=${cityName}&appid=bf7521dfb4b1e05765815bf0b82e5f0f`;
      const weatherData = await this.http.get<GeoLocation[]>(geoUrl).toPromise();

      if (weatherData && weatherData.length > 0) {
        const weatherUrl = `https://api.openweathermap.org/data/2.5/weather?lat=${weatherData[0]?.lat}&lon=${weatherData[0]?.lon}&appid=bf7521dfb4b1e05765815bf0b82e5f0f`;
